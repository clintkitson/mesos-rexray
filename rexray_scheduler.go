...
var (
	address      = flag.String("address", "172.31.1.11", "Binding address for artifact server")
...
func (sched *ExampleScheduler) ResourceOffers(driver sched.SchedulerDriver, offers []*mesos.Offer) {
	...
	for _, offer := range offers {
		...
		yamlOutput, err := yaml.Marshal(&offer)
		if err != nil {
			log.Fatal(err)
		}
		...
		for sched.tasksLaunched < sched.totalTasks &&
			...
			task := &mesos.TaskInfo{
				...
				Resources: []*mesos.Resource{
					util.NewScalarResource("cpus", CPUS_PER_TASK, role),
					util.NewScalarResource("mem", MEM_PER_TASK, role),
				},
			}
